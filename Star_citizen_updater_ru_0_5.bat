...
set "GITHUB_RAW=https://raw.githubusercontent.com/%GITHUB_AUTOR%/%GITHUB_REPO%/master"
...
if exist "%LAUNCHER_DIR%RSI Launcher.exe" (
    set "LAUNCHER_PATH=%LAUNCHER_DIR%"
    set "LAUNCHER_AUTO_DETECTED=1"
)

:: Проверка наличия StarCitizen_Launcher.exe рядом со скриптом (папка игры)
if exist "%LAUNCHER_DIR%StarCitizen_Launcher.exe" (
    if exist "%LAUNCHER_DIR%Bin64\StarCitizen.exe" (
        set "LIVE_PATH=%LAUNCHER_DIR%"
...
if "!LAUNCHER_PATH!"=="" (
    ...
    :: Если папка игры найдена, выводим информацию
    if not "!LIVE_PATH!"=="" (
        echo ✓ Найдена папка игры LIVE: !LIVE_PATH!
        echo.
    )
...
:: Если лаунчер был автоматически обнаружен, сохраняем конфигурацию
if defined LAUNCHER_AUTO_DETECTED (
    call :SaveConfig
    set "LAUNCHER_AUTO_DETECTED="
)

:: Если папка игры была автоматически обнаружена, сохраняем конфигурацию
...
powershell -NoProfile -Command "try { $response = Invoke-WebRequest -Uri 'https://api.github.com/repos/%GITHUB_AUTOR%/%GITHUB_REPO%/releases/latest' -UseBasicParsing; $json = $response.Content | ConvertFrom-Json; $tag = $json.tag_name; $version = 'не найдена'; if($tag -match '^(\d+\.\d+\.\d+)-(v\d+)$') { $version = $matches[1] + ' ' + $matches[2] }; \"VERSION:$version\" | Out-File -FilePath '%TEMP_DIR%\github_version.txt' -Encoding UTF8 } catch { 'VERSION:не найдена' | Out-File -FilePath '%TEMP_DIR%\github_version.txt' -Encoding UTF8; $_.Exception.Message | Out-File -FilePath '%TEMP_DIR%\github_debug.txt' -Encoding UTF8 }" >nul 2>&1
...
powershell -NoProfile -Command "try { Invoke-WebRequest -Uri 'https://github.com/%GITHUB_AUTOR%/%GITHUB_REPO%/archive/refs/heads/master.zip' -OutFile '%TEMP_DIR%\localization.zip' -UseBasicParsing; exit 0 } catch { exit 1 }" >nul 2>&1
...
if exist "%CONFIG_FILE%" (
    ...
    :: Проверяем, существуют ли папки (БЕЗ сканирования дисков)
    if not "!LIVE_PATH!"=="" (
        call :ValidateGameFolder "!LIVE_PATH!" "LIVE"
        if !ERRORLEVEL! neq 0 (
            echo ⚠ Папка LIVE не найдена по сохранённому пути
            set "LIVE_PATH="
        ) else (
            set "LIVE_FOUND=true"
        )
    ...
    if not "!PTU_PATH!"=="" (
        ...
        if !ERRORLEVEL! neq 0 (
            echo ⚠ Папка PTU не найдена по сохранённому пути
            set "PTU_PATH="
        ) else (
            set "PTU_FOUND=true"
        )
    ...
    :: Если папка игры найдена, но лаунчер нет, продолжаем (лаунчер будет запрошен позже)
...
:: Если лаунчер уже найден и не был автоматически обнаружен, спрашиваем хочет ли пользователь его переустановить
if not "!LAUNCHER_PATH!"=="" (
    if not defined LAUNCHER_AUTO_DETECTED (
        ...
        if /i not "!RECONFIGURE_LAUNCHER!"=="Y" (
...
:: Если папка LIVE была автоматически обнаружена, пропускаем выбор
if defined LIVE_AUTO_DETECTED (
...
:: Перерисовываем экран (подавляем ошибки)
call :RedrawScreen 2>nul
