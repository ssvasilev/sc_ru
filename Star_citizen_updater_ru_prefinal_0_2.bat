...
:: Функция извлечения версии из файла global.ini (больше не используется для парсинга версии)
:GetVersionFromFile
set "file_path=%~1"
set "return_var=%~2"
...
:: Ищем строку Frontend_PU_Version и извлекаем версию в формате "X.X.X vXX"
for /f "usebackq delims=" %%a in (`findstr /c:"Frontend_PU_Version" "%file_path%" 2^>nul`) do (
    ...
    :: Ищем фразу "Установленная версия: LIVE X.X.X vXX" или "Установленная версия: PTU X.X.X vXX"
    for /f "delims=" %%b in ('powershell -NoProfile -Command "if('!line!' -match 'Установленная версия:\s+(?:LIVE|PTU)\s+([\d\.]+\s+v\d+)') { $matches[1] } else { 'не найдена' }"') do (
        set "version=%%b"
    )
)
